/**
 * Acará Poker Club Helper — Cloudflare Worker backend (Gemini)
 *
 * Secrets / variables:
 *   GEMINI_API_KEY   (secret, required)
 *   ALLOWED_ORIGINS  (text variable, recommended)
 *     Example: https://bloodsieger642-design.github.io
...
export default {
  async fetch(request, env) {
    ...
    if (!originAllowed(request, env)) {
      return json(
        { error: "Origem não autorizada. Ajuste ALLOWED_ORIGINS no Worker." },
        403,
        request,
        env
      );
    ...
    if (!env.GEMINI_API_KEY) {
      return json(
        { error: "GEMINI_API_KEY não foi configurada no Worker." },
        500,
        request,
        env
      );
    ...
    if (len && len > MAX_REQUEST_BYTES) {
      return json(
        { error: "Requisição muito grande. Escolha uma foto menor." },
        413,
        request,
        env
      );
    ...
    if (!mediaType || !imageData) {
      return json(
        { error: "Imagem ausente ou formato não suportado." },
        400,
        request,
        env
      );
    ...
    if (imageData.length > MAX_IMAGE_BASE64_BYTES) {
      return json(
        { error: "Imagem muito grande para análise. Reduza a resolução." },
        413,
        request,
        env
      );
    ...
    try {
      ...
      return json(
        { error: "Falha de rede entre o Worker e o Gemini." },
        502,
        request,
        env
      );
    ...
    if (!apiResponse.ok) {
      ...
      return json({
        error: `Gemini respondeu HTTP ${apiResponse.status}${detail ? ": " + detail : ""}`,
      }, 502, request, env);
    }
    ...
    try {
      ...
      return json(
        { error: "Não foi possível interpretar a resposta do Gemini." },
        502,
        request,
        env
      );
